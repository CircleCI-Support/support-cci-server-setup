...
module "s3_bucket" {
  ...
  bucket = "${local.cluster_name}-circleci-dlc"
  acl    = "private"
...
module "circleci_dlc_pod_identity" {
  ...
  name = "${module.eks.cluster_name}-cci-dlc-pod-identity"
    ...
    jsonencode({
      ...
      Statement = [
        ...
        },
        {
          ...
          Resource = [
            "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${module.eks.cluster_name}-circleci-dlc*"
          ]
        }
      ]
    })
...
module "circleci_machine_provisioner_pod_identity" {
  ...
  name = "${module.eks.cluster_name}-cci-provisioner-pod-identity"
